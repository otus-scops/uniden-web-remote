...
| 変数名 | デフォルト値 | 説明 |
|:---|:---|:---|
| `AUDIO_FORMAT` | `mp3` | 録音フォーマット (現在は省ストレージ・ブラウザ互換性の観点から `mp3` に特化・固定) |
...
> [!NOTE]
> **🎙️ 音声エンジンとフォーマットに関する仕様**:
> - **SoX（Sound eXchange）の採用**:
>   本システムは、重厚な FFmpeg ではなく、軽量かつ極小バッファで ALSA と連携可能な **SoX** を音声ストリーミングおよび録音バックエンドとして採用しています。これにより、Raspberry Pi 等の小型PCでも低負荷で動作し、ブラウザへの 100〜300ms 超低遅延配信を実現しています。
> - **MP3 フォーマットへの最適化**:
>   24時間365日常時録音時のストレージ消費（32kbps Mono で約14.4MB/時）および、PC・スマホ（iOS Safari / Android Chrome）を問わないブラウザ直接再生の互換性を最大限に確保するため、現在の実装は `mp3` 出力に最適化されています。
